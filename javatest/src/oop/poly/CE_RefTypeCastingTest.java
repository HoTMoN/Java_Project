...
public class CE_RefTypeCastingTest {
	public static void main(String[] args) {
		System.out.println("1. Super타입의 참조변수로 Super객체를 엑세스");
		Parent obj1 = new Parent();
		obj1.display();
		System.out.println(obj1.name);
		System.out.println("*******************************************************");	
	
		System.out.println("2. Sub타입의 참조변수로 Sub객체를 엑세스");
		 ...
		 *     2) 변수를 통해서 엑세스할 수 있는 멤버는 변수의 타입으로 제한
		 *     	  변수가 부모타입이면 부모의 멤버만 접근할 수 있다.
		 *     3) 변수가 부모타입이라고 하더라도 실제 참조하는 객체가 자식타입이면
		 *     	  변수를 자식타입으로 캐스팅해서 접근할 수 있다.
		 */		
		System.out.println("3. Super타입의 참조변수로 Sub객체를 엑세스");
		Parent obj3 = new Child();
		obj3.display();
		//obj3은 Parent타입이므로 Parent의 멤버만 엑세스 할 수 있다.
		//그러나... 실제 Parent 타입의 변수가 참조하는 객체가 Child 타입이므로 casting(형변환)을 하면 Child객체의 멤버도 접근이 가능하다.
		//obj3을 Child타입으로 캐스팅
		((Child)obj3).show();
		System.out.println(obj3.name);
		System.out.println("*******************************************************");
		
		System.out.println("4. Sub타입의 참조변수로 Super객체를 엑세스--------X");
...
//		System.out.println(obj4.name);
		...
		//전혀 상관없는 클래스들(상속관계가 아닌클래스)은 캐스팅조차 할 수 없다.
		//----------------------
...
//		String str2 = (String)rand;
		...
		System.out.println("5. Sub타입의 참조변수 = Super객체를 참조하는 Super타입의 변수--------?");
//		Child obj5 = (Child)obj1;
//		obj5.display();
//		System.out.println(obj5.name);
		System.out.println("*******************************************************");
		
		System.out.println("6. Sub타입의 참조변수 = Sub객체를 참조하는 Super타입의 변수4");
