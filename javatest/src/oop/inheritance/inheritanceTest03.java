 ...
 *  2. 모든 클래스의 초상위 클래스는 java.lang.Object클래스이다.
 *		=> 다른 클래스를 상속하지 않은 클래스는 모두 Object클래스를 상속하는 명령문이 생략되어 있다.
...
class SubA extends SuperA{  //1.SubA에는 기본생성자가 생략되어 있다.
	int age;
	String addr;
	public SubA() { 	    //2.모든 생성자의 첫 번째 문장에는 기본생성자의 호출문이 생성되어 있다.
		super();				    //3.부모클래스에서는 매개변수가 있는 생성자가 이미 정의되어 있으므로 컴파일러가 기본생성자를 추가하지 않는다.
	}					    //  =>부모의 기본생성자를 호출하는데 부모클래스에는 기본생성자가 없으므로 오류가 발생
						    //  =>해결방법
							//	- 부모클래스에 기본생성자를 추가.
							//  - 부모클래스에 정의된 생성자를 호출하도록 변경
	SubA(String name,int age,String addr){//SubA가 name을 갖고 있지 않더라도 name변수를 전달받을 수 있도록 추가해야 한다.
		super(name); //부모에 정의되어 있는 String매개변수 한 개를 받는 생성자를 찾아서 호출한다.
					// 이런경우 부모의 기본생성자 호출문을 대신
		this.age = age;
		this.addr = addr;
	}
	public void display() {
...
public class inheritanceTest03 {
	public static void main(String[] args) {
		...
		//따라서 하위클래스를 사용하는 경우 하위클래스가 직접적으로 멤버변수를 갖고 있지 않다고 하더라도 모든 값을 하위클래스에서 받
		SubA obj2 = new SubA("장동건",20,"인천");
		obj.display(); // obj객체는 멤버의 변수 값이 아무것도 셋팅되어 있지 않으므로 모두 결과가 출력되지 않는다.
