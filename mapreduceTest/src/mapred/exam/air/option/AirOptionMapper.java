...
//하둡을 실행할떄 사용자가 입력(쉘 프롬포트에서 실행을 위해 사용자가 입력)하는 옵션을 Mapper내부에서
//사용할 수 있도록 처리 => 옵션이 어떤 값이 입력되었느냐에 따라 다른 작업을 할 수 있도록 처리
public class AirOptionMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
	...
	protected void setup(Mapper<LongWritable, Text, Text, IntWritable>.Context context)
		...
		/*
		 * 사용자가 입력하는 옵션 정보를 환경설정 정보에서 가져와서 변수 jobType에 셋팅 옵션명 : get메소드의 매개변수는 사용자가 입력하는
		 * 옵션명 -> jobType
		 */
	...
	protected void map(LongWritable key, Text value, Mapper<LongWritable, Text, Text, IntWritable>.Context context)
		...
		if (key.get() > 0) {
			...
			if (line != null & line.length > 0) { // 분리된 결과가 null이 아니거나 결과가 있을때만 작업
				// jobType의 값에 따라서(사용자가 입력하는 jobType옵션의 값에 따라서) 다르게 동작할 수 있도록 구현
				if (jobType.equals("departure")) {	// 출발지연
					if (!line[15].equals("NA") && Integer.parseInt(line[15]) > 0) {
						outputKey.set(line[1] + "월");// 월
						context.write(outputKey, outputval);
					}
				} else if (jobType.equals("arrival")) { //도착지연
